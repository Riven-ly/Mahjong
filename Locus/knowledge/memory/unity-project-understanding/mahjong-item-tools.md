...
injectMode: inherit
summary: Mahjong 撤销、洗牌与提示道具的运行时实现与入口缓存。
aiEditMode: inherit
---

- `MahjongGameLogic` 现已实现 `Undo()`、`Shuffle()`、`GetHintCardIds()` 与 `GetHintCardIdsForCard()`：撤销仅移出卡槽末尾的稳定牌并恢复为 `OnBoard`；洗牌只让 `OnBoard` 牌随机交换完整 `Layer + Position`；默认提示候选为所有可选同类型牌面对并循环轮换；已选一张牌时，提示优先查询该牌对应的一张可操作同类型牌。若不存在对应牌，`MahjongGameplayView.TryShowHint()` 会取消该牌选中状态后回退到默认提示查询。
