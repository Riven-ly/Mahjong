...
type: memory
path: unity-project-understanding/mahjong-level-randomization.md
title: mahjong-level-randomization
inheritInjectMode: true
summaryEnabled: true
commandEnabled: false
readOnly: false
inheritAiConfig: true
createdAt: 1786098336847
updatedAt: 1786157927524
---

# mahjong-level-randomization

## Summary
外部麻将布局坐标导入与运行时随机配对牌型实现记录。
...
- `MahjongLevelDefinition.randomizeTypeIds` 控制进入关卡时是否为固定牌位生成随机牌面。`MahjongLevelRandomizer` 先基于遮挡规则找出可取顺序，再以42种牌型按两张一组填入；牌对数量超过42时重新洗牌循环，允许同一牌型在一局中出现多对，从而保证可解路径。
