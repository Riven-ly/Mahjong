...
- `MahjongGameplayView` 对外提供 `TryUndo()`、`TryShuffle()`、`TryShowHint()`、`StopHint()`；所有道具操作均要求没有入槽或消除动画。洗牌后更新 UGUI 坐标、显示层级与阻挡状态；撤销后将末尾槽牌重新放回 BoardRoot，并调用 `RefreshBoardPositions()` 依层级重排全部牌面视图。不可仅将撤销牌 `SetAsLastSibling()`，否则洗牌后会让低层撤销牌覆盖高层牌，造成显示与遮挡规则不一致。
